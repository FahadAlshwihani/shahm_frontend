...
const IconTrash = () => (
  <svg width="15" height="15" viewBox="0 0 15 15" fill="none" aria-hidden="true">
    <path d="M2 4H13M5.5 4V2.5H9.5V4M6 7V11.5M9 7V11.5M3 4L3.75 12.5C3.75 13.05 4.2 13.5 4.75 13.5H10.25C10.8 13.5 11.25 13.05 11.25 12.5L12 4" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
  </svg>
);
...
export default function CMS_Footer() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const genRes = await api.get("public/settings/");
      setGeneralSettings(genRes.data);
    } catch { /* optional */ }
    try {
      const fsRes = await api.get("cms/admin/footer/settings/");
    ...
    try {
      const ctaRes = await api.get("cms/admin/footer/cta/");
  ...
  const handleToggleLinkActive = async (link) => {
    try {
      await api.patch(`cms/admin/footer-links/${link.id}/`, { is_active: !link.is_active });
  ...
  const submitFooterSettings = async (e) => {
    ...
    await new Promise((r) => setTimeout(r, 800));
    try {
      await api.post("cms/admin/footer/settings/", fd, {
        headers: { "Content-Type": "multipart/form-data" },
      });
  ...
  const submitCta = async (e) => {
    ...
    try {
      if (editCtaId) {
        await api.patch(`cms/admin/footer/cta/${editCtaId}/`, payload);
        toast.success(t("cms.footer.column_updated"));
      } else {
        await api.post("cms/admin/footer/cta/", payload);
        toast.success(t("cms.footer.column_created"));
      }
      resetCtaForm();
      loadData();
  ...
  const handleDeleteCta = async (id) => {
    ...
    await new Promise((r) => setTimeout(r, 800));
    try {
      await api.delete(`cms/admin/footer/cta/${id}/`);
      toast.success(t("cms.footer.link_deleted"));
      loadData();
    } catch {
      toast.error(t("cms.footer.link_save_failed"));
      ...
      <li className="cms-footer-link-item">
        <div className="cms-footer-link-content">
          ...
          <input
            ...
            onChange={async (e) => {
              try {
                await api.patch(`cms/admin/footer-links/${item.id}/`, { order: e.target.value });
                toast.success(t("cms.footer.order_updated"));
                loadData();
              } catch {
                toast.error(t("cms.footer.link_save_failed"));
                ...
                <div className="cms-footer-card-header">
                  ...
                  <div className="cms-footer-card-actions">
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        try {
                          await api.patch(`cms/admin/columns/${col.id}/`, { order: e.target.value });
                          toast.success(t("cms.footer.order_updated"));
                          loadData();
                        } catch {
                          toast.error(t("cms.footer.column_save_failed"));
