...
export default function CMS_Header() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editId) {
        await api.patch(`cms/admin/header/${editId}/`, fd, { headers:{ "Content-Type":"multipart/form-data" } });
        toast.success(t("cms.header.success.updated"));
      } else {
        await api.post("cms/admin/header/", fd, { headers:{ "Content-Type":"multipart/form-data" } });
  ...
  const handleDeleteLogo = async (variant) => {
    ...
    try {
      await api.delete(`cms/admin/header/${logoItem.id}/`);
  ...
  const handleMenuImageSubmit = async (e) => {
    ...
    try {
      if (menuImageEditId) {
        await api.patch(`cms/admin/header/${menuImageEditId}/`, fd, { headers:{ "Content-Type":"multipart/form-data" } });
        toast.success(t("cms.header.success.updated"));
      } else {
        await api.post("cms/admin/header/", fd, { headers:{ "Content-Type":"multipart/form-data" } });
        toast.success(t("cms.header.success.created"));
  ...
  const handleQaSubmit = async (e) => {
    ...
    try {
      if (qaEditId) {
        await api.patch(`cms/admin/header/${qaEditId}/`, fd, { headers:{ "Content-Type":"multipart/form-data" } });
        toast.success(t("cms.header.success.updated"));
      } else {
        await api.post("cms/admin/header/", fd, { headers:{ "Content-Type":"multipart/form-data" } });
        toast.success(t("cms.header.success.created"));
  ...
  const loadPageContent = async (slug) => {
    try {
      const res = await api.get(`cms/public/content/${slug}/`);
      setPcForm(res.data);
    } catch { toast.error(t("cms.header.save_failed")); }
  };
  ...
  const handlePcSubmit = async (e) => {
    ...
    setLoadingKey(key, true);
    try {
      await api.patch(`cms/admin/${pcEditSlug}/`, pcForm);
      toast.success(t("cms.header.success.updated"));
    } catch { toast.error(t("cms.header.save_failed")); }
    finally  { setLoadingKey(key, false); }
  };
  ...
  const renderTree = (items, level = 0) =>
    items.map((item) => {
      const isDeleting = loading[`menu-delete-${item.id}`];
      return (
          ...
          <div className="cms-header-tree-content">
            <div className="cms-header-tree-order">
              ...
              <input type="number" className="cms-header-input-number" value={item.order}
                ...
                onChange={async (e) => {
                  await api.patch(`cms/admin/header/${item.id}/`, { order: e.target.value });
        ...
        <div className="cms-header-tab-content" key="logos">
          <div className="cms-header-logo-grid">
            {logoVariants.map(({ key, label }, idx) => {
              ...
              const isDeleting  = loading[`logo-delete-${key}`];
              return (
                        ...
                        <label className="cms-header-btn-upload" style={{ cursor:"pointer" }}>
                          ...
                          <input type="file" accept="image/*" style={{ display:"none" }}
                            onChange={(e) => {
                              ...
                              api.post("cms/admin/header/", fd, { headers:{ "Content-Type":"multipart/form-data" } })
